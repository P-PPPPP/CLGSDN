...
class AGCN(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k):
        ...
        self.relu = nn.ReLU()
        self.mlp = nn.Linear(2*cheb_k*dim_in, dim_out)
        nn.init.xavier_normal_(self.mlp.weight)
        nn.init.constant_(self.mlp.bias, val=0)
        self.bn = nn.BatchNorm2d(2*cheb_k)
...
class MegaCRN(nn.Module):
    def __init__(self, configs):
        ...
        input_dim = configs['in_dim']
        output_dim = configs['out_dim']
        horizon = configs['pred_len']
        rnn_units = configs['rnn_units']
        num_layers = configs['num_rnn_layers']
        cheb_k = configs['max_diffusion_step']
        ycov_dim = 1
        mem_num = configs['mem_num']
        mem_dim = configs['mem_dim']
        cl_decay_steps = configs['cl_decay_steps']
        use_curriculum_learning = configs['use_curriculum_learning']
        # num_layers=1,
        # cheb_k=3,
        # ycov_dim=1, 
        # mem_num=20, 
    ...
    def forward(self, x, y_cov, labels=None, batches_seen=None, **args):
