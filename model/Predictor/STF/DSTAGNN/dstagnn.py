...
class cheb_conv_withSAt(nn.Module):
    ...
    def __init__(self, K, cheb_polynomials, in_channels, out_channels, num_of_vertices, with_GraphGen=False):
        '''
        :param K: int
    ...
    def forward(self, x, spatial_attention, adj_pa, adjs):
        '''
...
class DSTAGNN_block(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                 cheb_polynomials, adj_pa, adj_TMD, num_of_vertices, num_of_timesteps, d_model, d_k, d_v, n_heads, with_GraphGen):
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(K, cheb_polynomials, in_channels, nb_chev_filter, num_of_vertices, with_GraphGen=with_GraphGen)
    ...
    def forward(self, x, res_att, adjs):
        '''
        :param x: (Batch_size, N, F_in, T)
        ...
        spatial_gcn = self.cheb_conv_SAt(x, STAt, self.adj_pa, adjs)  # B,N,F,T
...
class DSTAGNN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                 cheb_polynomials, adj_pa, adj_TMD, num_for_predict, len_input, num_of_vertices, d_model, d_k, d_v, n_heads,with_GraphGen):
        '''
        :param nb_block:
        :param in_channels:
        :param K:
        ...
        self.BlockList = nn.ModuleList([DSTAGNN_block(DEVICE, num_of_d, in_channels, K,
                                                     ...
                                                     adj_pa, adj_TMD, num_of_vertices, len_input, d_model, d_k, d_v, n_heads,with_GraphGen)])
        ...
        self.BlockList.extend([DSTAGNN_block(DEVICE, num_of_d * nb_time_filter, nb_chev_filter, K,
                                            ...
                                            adj_pa, adj_TMD, num_of_vertices, len_input//time_strides, d_model, d_k, d_v, n_heads,with_GraphGen) for _ in range(nb_block-1)])
    ...
    def forward(self, x, adjs=None):
        ...
        res_att = 0
        for block in self.BlockList:
            x, res_att = block(x, res_att, adjs)
