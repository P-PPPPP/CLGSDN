...
class DCRNNEncoder(BaseModel):
    def __init__(self, input_dim, adj_mat, max_diffusion_step, hid_dim, num_nodes,
                 num_rnn_layers, filter_type,device='cpu'):
    ...
    def forward(self, inputs, initial_hidden_state, adjs=None):
        ...
        for i_layer in range(self._num_rnn_layers):
            ...
            for t in range(seq_length):
                _, hidden_state = self.encoding_cells[i_layer](current_inputs[t, ...], hidden_state, adjs)  # (50, 207*64)
                output_inner.append(hidden_state)
            output_hidden.append(hidden_state)
            current_inputs = torch.stack(output_inner, dim=0).to(inputs.device)  # seq_len, B, ...
...
class DCGRUDecoder(BaseModel):
    def __init__(self, input_dim, adj_mat, max_diffusion_step, num_nodes,
                 hid_dim, output_dim, num_rnn_layers, filter_type,device='cpu'):
    ...
    def forward(self, inputs, initial_hidden_state, teacher_forcing_ratio=0.5, adjs=None):
        """
        ...
        for t in range(1, seq_length):
            ...
            for i_layer in range(0, self._num_rnn_layers):
                hidden_state = initial_hidden_state[i_layer]
                output, hidden_state = self.decoding_cells[i_layer](current_input, hidden_state, adjs)
            ...
            teach_bool = True
            current_input = (inputs[t] if teach_bool and teacher_force and self.training else output)

        return outputs


class DCRNNModel(BaseModel):
    def __init__(self, adj_mat, configs, with_GraphGen):
                # batch_size, enc_input_dim, dec_input_dim, max_diffusion_step, num_nodes,
        ...
        self._num_nodes = configs['nodes']  # should be 207
        self._num_rnn_layers = configs['num_rnn_layers']  # should be 2
        self._rnn_units = configs['rnn_units']  # should be 64
        self._seq_len = configs['seq_len']  # should be 12
        # use_curriculum_learning = bool(model_kwargs.get('use_curriculum_learning', False))  # should be true
        self._output_dim = configs['output_dim']  # should be 1

        batch_size = configs['batch_size']
        num_nodes = self._num_nodes
        enc_input_dim = configs['enc_input_dim']
        max_diffusion_step = configs['max_diffusion_step']
        rnn_units = configs['rnn_units']
        num_rnn_layers = configs['num_rnn_layers']
        filter_type = configs['filter_type']
        dec_input_dim = configs['dec_input_dim']

        # specify a GO symbol as the start of the decoder
        self.GO_Symbol = torch.zeros(1, batch_size, num_nodes * self._output_dim, 1).to(configs['device'])

        self.encoder = DCRNNEncoder(input_dim=enc_input_dim, adj_mat=adj_mat,
                                    max_diffusion_step=max_diffusion_step,
                                    hid_dim=rnn_units, num_nodes=num_nodes,
                                    num_rnn_layers=num_rnn_layers, filter_type=filter_type,device=configs['device'])
    ...
    def forward(self, source, target, teacher_forcing_ratio, adjs):
        # the size of source/target would be (64, 12, 207, 2)
        source = source.permute(3,0,2,1)
        target = target.permute(3,0,2,1) # torch.transpose(target[..., :self._output_dim], dim0=0, dim1=1)
        ...
        context, _ = self.encoder(source, init_hidden_state, adjs)  # (num_layers, batch, outdim)

        outputs = self.decoder(target, context, teacher_forcing_ratio=teacher_forcing_ratio, adjs=adjs)
