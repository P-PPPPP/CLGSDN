...
class DiffusionGraphConv(BaseModel):
    def __init__(self, supports, input_dim, hid_dim, num_nodes, max_diffusion_step, output_dim, bias_start=0.0):
        ...
        self._supports = [s@torch.eye(s.size(0),device=s.device) for s in supports]
        ...
        self.relu = nn.ReLU()
        # projection
        self.mlp = nn.Linear((2+1)*(input_dim+hid_dim), output_dim)
    ...
    def spatial_conv(self, x, adjs):
        B,N,D = x.size()
        out = [x.unsqueeze(1)]
        # Graph Convolution
        x1 = torch.einsum('BHNM,BND->BHND',(adjs,x))
        out.append(x1)
        h = torch.cat(out,dim=1).transpose(1,2)
        # h = h.reshape(B,N,-1)
        # h = self.mlp(h)
        # h = F.tanh(h)
        # h = self.dp(h)
    ...
    def forward(self, inputs, state, output_size, bias_start=0.0, adjs=None):
        """
        ...
        if adjs is None:
            if self._max_diffusion_step == 0:
                ...
                for support in self._supports:
                    x1 = torch.einsum('NM,NDB->MDB',(support,x0))
                    x = self._concat(x, x1)
                    for k in range(2, self._max_diffusion_step + 1):
                        x2 = 2 * torch.einsum('NM,NDB->MDB',(support,x0)) - x0
                        x = self._concat(x, x2)
                        x1, x0 = x2, x1
        else:
            x = self.spatial_conv(inputs_and_state,adjs)
            x = x.permute(2,1,3,0)
        # [order,N,D,B]
        ...
        y = torch.reshape(x, [batch_size, self._num_nodes, output_size])
...
class DCGRUCell(BaseModel):
    ...
    def forward(self, inputs, state, adjs):
        """
        ...
        if self._use_gc_for_ru:
            ...
            fn = self._fc
        value = torch.sigmoid(fn(inputs, state, output_size, bias_start=1.0, adjs=adjs))
