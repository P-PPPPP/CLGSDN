...
class autoformer_api(nn.Module):
    def __init__(self, configs, graph_generator, fixed_adjs):
        super(autoformer_api, self).__init__()
        ...
        self.model = AUTOFORMER_Model(self.configs)
        self.model_init()
    ...
    def load_configs(self,configs):
        ...
        model_configs['dec_in'] = configs['exp']['c_in']
        model_configs['enc_in'] = configs['exp']['c_in']
        model_configs['seq_len'] = configs['exp']['inp_len']
        model_configs['label_len'] = model_configs['seq_len'] // 2
    ...
    def forward(self, seq_x, seq_x_mark, seq_y_mark,**args):
        ...
        predicts,_ = self.model(x_enc, x_enc_mark, x_dec, x_dec_mark)
    ...
    #     # for multi nodes
    #     assert x.shape[1] == 1
    #     x_enc = x.squeeze(1).transpose(1,2)
    #     x_enc_mark = x_mark.squeeze(1)
    #     x_dec = y.squeeze(1).transpose(1,2)
    #     x_dec_mark = y_mark.squeeze(1)
    #     x_dec = torch.cat((x_enc[:,-self.configs['label_len']:,:],x_dec),dim=1)
    #     x_dec_mark = torch.cat((x_enc_mark[:,-self.configs['label_len']:,:],x_dec_mark),dim=1)
    #     return x_enc,x_dec,x_enc_mark,x_dec_mark