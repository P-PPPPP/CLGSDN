...
from model.CLGSDN.CLGSDN_GraphMemoryBlock import Graph_LSTM,Graph_GRU
from model.CLGSDN.Time_series_similarity import time_delay_self_attn,dist_measurements
from model.Universal.My_functional import muti_heads_attn,denoising_filter_channels,linear_as_conv2d,scale_hard_sigmoid,scale_hard_tanh
...
class probability_model(nn.Module):
    ...
    def LPM(self):
        ...
        global_view = global_view.repeat(self.configs['batch_size'],1,1,1)  # np.save('saved_graph/nega_LPM',self.learnable_matrix.cpu().detach().numpy())
        return global_view
