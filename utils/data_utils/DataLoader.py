...
class Dataset_with_Time_Stamp(Dataset):
    ...
    def set_parameters(self, configs, flag):    
        ...
        self.channels = configs['dataset']['n_channels']
        self.c_in = configs['exp']['c_in']
        self.period_type = configs['exp']['period_type']
        self.inp_len = configs['exp']['inp_len']
        self.pred_len = configs['exp']['pred_len']
        self.seq_len = self.inp_len +self.pred_len
        # init
        assert flag in ['train', 'test', 'vali']
        type_map = {'train': 0, 'vali': 1, 'test': 2}
        self.set_type = type_map[flag]
        self.data_scale = configs['exp']['data_scale']
        self.date_scale = configs['exp']['date_scale']
        self.freq = configs['dataset']['time_window']
        self.dtype = configs['exp']['dtype']
        self.choice_channels = configs['exp']['select_channels']
        if sum(configs['exp']['dataset_prob']) != 1.0:
            self.dataset_prob = [i/sum(configs['exp']['dataset_prob']) for i in configs['exp']['dataset_prob']]
        else:
            self.dataset_prob = configs['exp']['dataset_prob']
    ...
    def __read_data__(self):
        ...
        if self.data_scale: 
            ...
            # assert N == self.num_nodes and C == self.c_in, 'Tensor axis error: make sure that your axis is [n time windows, nodes, channels]'
        ...
        if self.date is None:
            self.data_stamp_x = torch.arange(0,self.inp_len)
            self.data_stamp_y = torch.arange(self.inp_len,self.inp_len+self.pred_len)
        else:
    ...
    def to(self, device='cpu',dtype=None):
        ...
        if self.date is not None:
            ...
            self.data_stamp_x = self.data_stamp_x.to(device=device,dtype=dtype)
            self.data_stamp_y = self.data_stamp_y.to(device=device,dtype=dtype)
...
class DataScaler():
    ...
    def trans_trans(self, data, choise_channels):
    ...
    def inv_trans_wo_mean(self, data, choise_channels):
        # [Batch, channels, nodes, time windows]
        std = self.std[self.channels].reshape(1,len(choise_channels),1,1)
        # mean = self.mean[self.channels].reshape(1,len(choise_channels),1,1)
        data = data * std
