...
class MutiProcess_tasks():
    ...
    def run_task(self, *configs):
        ...
        setting_seed(configs['exp']['seed']) # 42: all the truths of the universe
        ...
        # 向实验框架中加载模型及数据
        exp.load(model, dataloader, dataset, scaler, loss=configs['exp']['loss'], optimizer='adam')
    ...
    def load_tasks(self, process_list, configs_list, task_load_flag_list, task_running_flag_list):
        ...
        while True:
            ...
            for i,item in enumerate(unfinish_tasks):
                ...
                # get free memorty
                device = task_configs['exp']['device']
